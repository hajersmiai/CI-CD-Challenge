...
async def translate_text(text: str, target_language: str, api_key: str) -> str:
    ...
    """
    if not api_key:
        st.error("API Key not found. Please provide GEMINI_API_KEY as an environment secret or in .env")
...
def app(env: str = "dev"):
    """Streamlit sub-application. The background is managed by main according to `env`.
    This function performs the UI and translation work.
    """
    env = (env or "dev").lower()

    # Message d'état selon l'environnement
    if env == "dev":
        st.info("Dev mode: verbose logs, test widgets enabled.")
    elif env == "qa":
        st.warning("QA mode: enhanced validations and test report.")
    else:
        st.error("Production mode: restricted and monitored operations.")
    ...
    with col1:
        ...
        language_select = st.selectbox(
            label="",
            options=("English", "Spanish", "French", "German", "Japanese", "Chinese (Simplified)", "Arabic"),
            index=0,
        )
    ...
    api_key = os.getenv("GEMINI_API_KEY", _DEFAULT_API_KEY)

    with col2:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("Translate", use_container_width=True, type="primary"):
            if not detective_input or not detective_input.strip():
                st.warning("Please enter some text to translate.")
            else:
                with st.spinner("Translating..."):
                    # Execute async properly inside Streamlit
                    result = asyncio.run(translate_text(detective_input.strip(), language_select, api_key))
